...
function createSocialCard() {
  ...
  drawText(image, 'DEV/TAB 3', 112, 150, 18, text)
  drawText(image, 'KEYBOARD-FIRST WORKSPACE', 118, 330, 6, accent)
  drawText(image, 'WORKSPACES  TASKS  FOCUS  SESSIONS', 118, 430, 4, muted)
...
function createDemoFrames() {
  const search = baseDemoFrame('WORKSPACES')
  frameRect(search, 58, 150, 450, 48)
  drawIndexedText(search, 'PICK UP WHERE YOU LEFT OFF', 76, 166, 2, 4)
  ...
  drawIndexedText(search, 'WORKSPACE', 76, 245, 2, 4)
  drawIndexedText(search, 'FOCUS', 272, 245, 2, 4)
  ...
  drawIndexedText(command, 'WORKSPACE DEVTAB', 164, 217, 2, 4)
  fillIndexed(command, 150, 265, 448, 52, 5)
  drawIndexedText(command, 'CONTEXT RESTORED', 164, 278, 2, 2)

  const notes = baseDemoFrame('TASKS')
  frameRect(notes, 58, 142, 145, 205)
  frameRect(notes, 218, 142, 402, 205)
  drawIndexedText(notes, 'OPEN', 76, 162, 2, 3)
  drawIndexedText(notes, 'DONE', 76, 202, 2, 4)
  drawIndexedText(notes, 'SHIP DEVTAB 3', 246, 166, 2, 3)
  ;['TEST WORKSPACE', 'BUILD EXTENSION', 'PUSH'].forEach((item, index) => {
    frameRect(notes, 246, 210 + index * 36, 18, 18, index === 0 ? 2 : 5)
    drawIndexedText(notes, item, 278, 212 + index * 36, 2, 4)
  })

  const customize = baseDemoFrame('SESSIONS')
  frameRect(customize, 58, 142, 562, 205)
  drawIndexedText(customize, 'CAPTURE  RESTORE  LAUNCH', 84, 170, 2, 4)
